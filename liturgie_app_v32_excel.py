...
APP_VERSION = "2026.08.25-persistant-supabase-v3.2-excel"
...
def xlsx_bytes(rows, state, title="Programme liturgique"):
    ...
    date_fmt = workbook.add_format({"align": "center", "valign": "top", "border": 1})
    cell_fmt = workbook.add_format({"valign": "top", "text_wrap": True, "border": 1})
    ...
    widths = [13, 18, 18, 18, 25, 25, 24, 24, 22, 22]
    for col, width in enumerate(widths):
        ws.set_column(col, col, width)
    if rows_flat:
        ws.autofilter(2, 0, 2 + len(rows_flat), len(fields) - 1)
    ws.freeze_panes(3, 1)
    ws.set_landscape()
    ws.fit_to_pages(1, 0)
